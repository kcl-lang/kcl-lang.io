...
| 仓库 | 语言 | 职责 |
|---|---|---|
| [kcl-lang/kcl](https://github.com/kcl-lang/kcl) | Rust | 编译器核心（`kclvm`）：解析、语义分析、求值器，以及 `kclvm_cli` 驱动二进制和 `libkclvm_cli_cdylib` 动态库。同时发布 `kcl-language-server`、`kcl-fmt`、`kcl-vet` 和 `kcl-test`。 |
| [kcl-lang/lib](https://github.com/kcl-lang/lib) | Go（内嵌） | 为各支持的操作系统/架构预编译的 `libkclvm_cli` 动态库，以 `kcl-lang.io/lib` Go module 的形式发布。 |
...
| [kcl-lang/kpm](https://github.com/kcl-lang/kpm) | Go | KCL 包管理器（`kcl mod ...`）。依赖 kcl-go，因此传递依赖 libkclvm。 |
...
- **所有执行路径最终都落到 Rust 核心。** 每一条 `kcl` 命令、每一次 Go API 调用、每一次 IDE 操作，最终都由 `kcl-lang/kcl` 中的求值器完成——要么通过 `kclvm_cli` 子进程，要么通过 cgo + `libkclvm_cli_cdylib`。
- **kcl-go 是 Go 生态的唯一集成点。** kpm、`kcl` CLI 以及 kcl-operator 等第三方工具都依赖它，因此在 kcl-go 中的 SDK 级修复会随下一次发布传播到所有下游工具。
- **版本号同步演进。** 各 Go module（`kcl-lang.io/kcl-go`、`kcl-lang.io/kpm` 等）的发布版本与 KCL 编译器版本保持一致，因此 `kcl-go v0.12.x` 搭配 `v0.12.x` 的编译器核心及 kcl-lang/lib 中对应的 `libkclvm` 二进制。
- **不同改动应该提到哪里：**
