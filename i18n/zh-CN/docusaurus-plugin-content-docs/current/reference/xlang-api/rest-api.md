...
> **在寻找跨语言 FFI 契约？**
> 请参阅 [`./ffi-abi.md`](./ffi-abi.md) 以了解 `call_native`、plugin-agent、`"ERROR:"` 前缀、缓冲区大小、`KCL_LIB_HOME` 以及导出的 Rust 符号。下文所述的 REST 接口是该契约的一个使用者。

## 1. 启动 REST 服务

RestAPI 服务可以通过以下方式启动：
...
POST 请求与返回的 JSON 数据均与 Protobuf 定义的结构保持一致。

## 2. `BuiltinService`

其中 `/api:protorpc/BuiltinService.Ping` 路径表示 `BuiltinService` 服务的 `Ping` 方法。

完整的 `BuiltinService` 由 Protobuf 定义如下：
...
`Ping` 方法可用于验证服务是否正常工作，而 `ListMethod` 方法可以查询所有提供的服务和函数的列表。

## 3. `KclService`

`KclService` 服务是与 KCL 功能相关的服务，其用法与 `BuiltinService` 服务相同。
...
然后我们希望使用 `Person` 来校验以下 JSON 数据：

```json
{ "key": "value" }
```

该校验可以通过 `KclService` 服务的 `ValidateCode` 方法完成。请参考 `ValidateCode` 方法的 `ValidateCodeArgs` 结构：
...
按照 `ValidateCodeArgs` 结构构造 JSON 数据，其中包含 `Person` 定义以及待校验的 JSON 数据：
...
将该 JSON 数据保存到 `vet-hello.json` 文件，并使用以下命令进行校验：
...
通过 Protobuf 定义的跨语言 API（[https://github.com/kcl-lang/kcl/blob/main/crates/api/spec.proto](https://github.com/kcl-lang/kcl/blob/main/crates/api/spec.proto)）：
