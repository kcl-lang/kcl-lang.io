...
+ 为已有的 Kubernetes 配置管理工具如 Kustomize, Helm 等提供**额外的能力增强如配置校验和编辑**等而不是替代它们，比如 Helm KCL Plugin, KRM Plugin 等
...
+ **K8s集成**：支持外置编辑和校验插件，分离数据和逻辑：[Helm KCL Plugin](https://github.com/KusionStack/helm-kcl), [KPT KCL SDK](https://github.com/KusionStack/kpt-kcl-sdk) 和 Kustomize KRM Plugin 等。
...
+ 如果你希望以现代语言方式编写复杂类型和建模，维护可扩展的配置文件，原生的纯函数和策略，和生产级的性能和自动化，建议直接选择 KCL 或将 KCL 用于对已有配置手段或工具进行增强
