...
The leading-underscore convention in schema attribute names is purely about **output visibility**: attribute names that begin with `_` are filtered out of the rendered YAML/JSON output unless the renderer is invoked with `--show-hidden`. It does **not** mark an attribute as immutable. Both regular and `_`-prefixed attributes may be reassigned later in the schema body:
