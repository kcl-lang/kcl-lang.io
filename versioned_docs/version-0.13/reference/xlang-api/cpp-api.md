...
> the **4 MiB** result buffer, and the `cxx::bridge` namespace
> `kcl_lib`. Every typed helper in `kcl_lib.hpp` is a thin C++ wrapper
> generated by [`cxx`](https://cxx.rs/) over that single dispatcher.

The [C++ API](https://github.com/kcl-lang/lib/tree/main/cpp) is built
with the [`cxx`](https://cxx.rs/) bridge over `kcl_api::call` and
exposes the full `KclService` + `BuiltinService` surface in the
`kcl_lib::` namespace. All `*Args` / `*Result` types are generated from
`spec/spec.proto` and every typed wrapper returns `Result<T>` that
maps the dispatcher `"ERROR:..."` reply into `kcl_lib::KclError`.
...
they are no longer recognised by the `kcl_lib::` dispatcher. If you
previously called `kcl_lib::build_program(...)` or
`kcl_lib::exec_artifact(...)`, switch to `kcl_lib::exec_program(...)`.
